...
# Constants
FPS = 30
DURATION = 15  # Total duration of video in seconds
WIDTH = 720
HEIGHT = 1280
R = 250
MAP_WIDTH = 139
...
class Projection:
    ...
    def display(self, message=None, message_center=None):
        ...
        if message:
            wrapped_message = self.wrap_text(message, font_large, WIDTH - 40)
            for idx, line in enumerate(wrapped_message):
                ...
                y_position = (HEIGHT // 2) + idx * line_height
                self.screen.blit(
                    text_surface,
                    text_surface.get_rect(center=(self.width // 2, y_position)),
                )
